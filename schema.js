    ...
    fields: () => {
        return {
            users: {
                ...
                resolve(root, { sorting, pageNum, numOfRecords, sortColumn }) {
                    let offset = (pageNum - 1) * numOfRecords;
                    return conn.models.user.findAll({ offset: offset, limit: numOfRecords, order: [[sortColumn, sorting]], where: { isDeleted: false } });
                }
            },
            user: {
            ...
            countUsers: {
                type: GraphQLInt,
                resolve(root, args) {
                    return conn.models.user.count({ where: { isDeleted: false } });
                }
            }
        }
    }
